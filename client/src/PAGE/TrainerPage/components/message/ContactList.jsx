...
export default function ContactList({ userData, socket, refresh }) {
  ...
  const fetchContacts = async () => {
    try {
      const res = await axios.get(`${API_URL}/trainer/chats`, {
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (newMessage) => {
      if (newMessage.chat_id === contactId) {
        ...
        // auto seen
        axios.put(
          `${API_URL}/trainer/chats/${contactId}/seen`,
          {},
          { withCredentials: true }
        );
      } else {
  ...
  const handleSelectContact = async (chatId) => {
    try {
      ...
      await axios.put(
        `${API_URL}/trainer/chats/${chatId}/seen`,
        {},
        { withCredentials: true }
      );

      const res = await axios.get(`${API_URL}/trainer/${chatId}`, {
        withCredentials: true,
      });
