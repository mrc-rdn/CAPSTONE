...
export default function ContactList({ userData, socket, refresh }) {
  // ... (All your existing state and functions remain exactly as you wrote them)
  ...
  const fetchContacts = async () => {
    try {
      const res = await axios.get(`${API_URL}/trainer/chats`, { withCredentials: true });
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (newMessage) => {
      if (newMessage.chat_id === contactId) {
        ...
        axios.put(`${API_URL}/trainer/chats/${contactId}/seen`, {}, { withCredentials: true });
      } else { fetchContacts(); }
    };
  ...
  const handleSelectContact = async (chatId) => {
    try {
      ...
      await axios.put(`${API_URL}/trainer/chats/${chatId}/seen`, {}, { withCredentials: true });
      const res = await axios.get(`${API_URL}/trainer/${chatId}`, { withCredentials: true });
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      await axios.post(`${API_URL}/trainer/sendMessage`, { chat_id: contactId, message }, { withCredentials: true });
      setMessage("");
    ...
    <div className="flex h-full gap-6 p-6">
      ...
      <aside className="w-80 flex flex-col bg-slate-50/50 rounded-[2rem] border border-slate-200/60 overflow-hidden shadow-sm">
        <div className="h-16 flex items-center px-6 border-b border-slate-200/60 bg-white/40">
          <h3 className="font-black text-slate-800 tracking-tight">Contacts</h3>
        </div>
        ...
        {contactId ? (
          <>
            ...
            <div className="h-16 border-b border-slate-200/60 bg-white/60 backdrop-blur-md flex items-center px-8 justify-between">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-2xl bg-emerald-50 flex items-center justify-center border border-emerald-100 shadow-sm text-emerald-700 font-black">
                  {(() => {
            ...
            <form onSubmit={handleSendMessage} className="p-6 bg-white/60 backdrop-blur-md border-t border-slate-200/60">
              <div className="flex items-center gap-3 bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm focus-within:ring-2 focus-within:ring-emerald-500/10 transition-all">
                <input
                  type="text"
                ...
                <button
                  ...
                  className={`w-11 h-11 rounded-xl flex items-center justify-center transition-all ${
                    message.trim() ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-200 hover:bg-emerald-700' : 'bg-slate-100 text-slate-300 cursor-not-allowed'
                  }`}
                >
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            <div className="w-20 h-20 bg-emerald-50 rounded-[2rem] flex items-center justify-center mb-6 border border-emerald-100 shadow-sm">
              <SendIcon sx={{ fontSize: 32, color: '#059669', transform: 'rotate(-45deg)' }} />
            </div>
            <h3 className="text-xl font-black text-slate-800 tracking-tight mb-2">Trainer Messaging</h3>
