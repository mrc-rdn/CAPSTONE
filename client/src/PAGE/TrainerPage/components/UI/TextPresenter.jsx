...
export default function TextPresenter({ data, courseId, chapterInfo }) {
        ...
        >
          {posts.map(post => (
            <SwiperSlide key={post.id} className="flex justify-center">
              <div className="w-full h-full border p-4 bg-white shadow hover:shadow-lg transition relative">

                {/* EDIT BUTTON */}
                <button
                  onClick={() => setEditingPost(post)}
                  className="absolute top-3 right-3 px-3 py-1 text-sm rounded-md bg-[#2D4F2B] text-white hover:bg-[#708A58]"
                >
                  Edit
                </button>
