...
export default function EditUserInfo(props) {
    ...
    const sendData = async (e) => {
        ...
        if (password === confirmPassword) {
            const res = await axios.post(`${API_URL}/trainer/edituserinfo`, { firstName, surname, contactNo, password }, { withCredentials: true })
    ...
    return (
        ...
        <div className="w-full h-full p-8 overflow-y-auto custom-scrollbar">
            <div className="max-w-4xl mx-auto bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl rounded-[3rem] border border-slate-200/60 dark:border-slate-800 shadow-xl shadow-slate-200/40 dark:shadow-none overflow-hidden transition-colors duration-300">
                ...
                {/* Header Banner */}
                <div className="h-32 bg-gradient-to-r from-emerald-600 to-emerald-800 p-8 flex items-end">
                    <h2 className="text-2xl font-black text-white tracking-tight uppercase">Profile Settings</h2>
                </div>
                    ...
                    <form className="space-y-8">
                        ...
                        <div className="flex flex-col items-center sm:items-start mb-8">
                            <div className="relative group cursor-pointer" onClick={() => props.handleUploadProfile()}>
                                ...
                                <div className="absolute bottom-0 right-0 bg-white dark:bg-slate-800 text-emerald-600 dark:text-emerald-400 w-10 h-10 rounded-2xl flex items-center justify-center shadow-lg border border-slate-100 dark:border-slate-700 group-hover:rotate-12 transition-transform">
                        ...
                        {(isUpdateProfile || isNotice || error) && (
                            <div className={`p-4 rounded-2xl text-center text-[10px] font-black uppercase tracking-[0.2em] border ${
                                ...
                                {isUpdateProfile ? "Profile Updated Successfully" : (notice || error)}
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    className="w-full h-12 bg-slate-50/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl px-5 text-sm font-bold text-slate-700 dark:text-slate-200 focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500/20 transition-all outline-none"
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    className="w-full h-12 bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl px-5 text-sm font-bold text-slate-400 dark:text-slate-500 cursor-not-allowed outline-none"
                                    type="text" disabled value={username}
                                />
                            </div>

                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1 text-emerald-600 dark:text-emerald-400">Password Recovery</label>
                                <div className="h-12 flex items-center px-5 bg-emerald-50 dark:bg-emerald-950/20 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
                                    <p className="text-[10px] font-black text-emerald-700 dark:text-emerald-400 uppercase">Secure Encryption Active</p>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between pt-10 border-t border-slate-100 dark:border-slate-800 gap-4">
                            <Link to="/trainer/dashboard">
                                <button className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                                    Cancel Changes
                                </button>
                            </Link>
                            ...
                            <button
                                onClick={sendData}
                                className="w-full sm:w-auto px-12 h-14 bg-emerald-600 text-white font-black text-xs uppercase tracking-[0.2em] rounded-[1.5rem] shadow-xl shadow-emerald-200 dark:shadow-none hover:bg-emerald-700 active:scale-95 transition-all"
                            >
                                Save Profile
                            </button>
                        </div>
