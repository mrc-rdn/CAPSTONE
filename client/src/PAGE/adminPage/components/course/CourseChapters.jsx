    ...
    const fetchingChapters = async () => {
      try {
        const [chapterInfo, chapterItems] = await Promise.all([
          axios.get(`${API_URL}/admin/course/${props.courseId}`, { withCredentials: true }),// this to fetch all the chapters
          axios.get(`${API_URL}/admin/course/1/${props.courseId}`, { withCredentials: true })
        ]);
