...
export default function SelectedProfile(props) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Points to admin endpoint
      const response = await axios.post(`${API_URL}/admin/addContact`, { user2_id: props.id }, { withCredentials: true })
      ...
      <div className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl border border-slate-200/60 overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 pb-0">
          <div className="flex flex-col">
             <h2 className="text-xl font-black text-slate-800 tracking-tight">User Details</h2>
             {isAdded && <p className="text-[10px] font-bold text-[#2D4F2B] uppercase tracking-widest">{isAdded}</p>}
          </div>
          ...
          <button
            ...
            className={`w-full mt-8 py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all ${
              isContactAdded 
                ...
                : 'bg-[#2D4F2B] text-white shadow-lg shadow-emerald-900/10 hover:bg-[#1e361d] active:scale-95'
            }`}
          >
            {isContactAdded ? 'Contact Linked' : 'Link to Contacts'}
          </button>
        </div>
      </div>
    </div>
