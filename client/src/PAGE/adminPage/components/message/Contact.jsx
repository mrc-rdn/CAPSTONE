...
export default function Contact(props) {
  ...
  return (
    <div
      className={
        props.isActive
          ? "w-full h-16 flex items-center px-4 mb-2 rounded-2xl bg-indigo-600 text-white shadow-lg shadow-indigo-200/50 dark:shadow-indigo-900/20 transition-all duration-200 scale-[1.02]"
          : "w-full h-16 flex items-center px-4 mb-2 rounded-2xl bg-white/50 dark:bg-slate-800/40 hover:bg-white dark:hover:bg-slate-800 border border-slate-200/60 dark:border-slate-700/50 transition-all duration-200 cursor-pointer shadow-sm hover:shadow-md text-slate-900 dark:text-slate-100"
      }
    ...
    >
      <div className="relative flex-shrink-0">
        {props.profile ? (
          <img
            src={props.profile}
            alt=""
            className={`w-11 h-11 rounded-full border-2 object-cover ${props.isActive ? 'border-white/40' : 'border-indigo-100 dark:border-slate-700'}`}
          />
        ) : (
          <div
        ...
        {props.unread_count > 0 && (
          <span className="min-w-[18px] h-[18px] bg-red-500 text-white text-[10px] rounded-full absolute -top-1 -right-1 flex items-center justify-center shadow-lg border-2 border-white dark:border-slate-900 px-1 font-black">
            {props.unread_count}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-col ml-3 overflow-hidden">
        <p className={`font-black text-sm truncate tracking-tight ${props.isActive ? 'text-white' : 'text-slate-800 dark:text-slate-200'}`}>
          {props.firstName} {props.surname}
        </p>
        <p className={`text-[10px] font-bold uppercase tracking-wider truncate ${props.isActive ? 'text-indigo-50' : 'text-slate-400 dark:text-slate-500'}`}>
