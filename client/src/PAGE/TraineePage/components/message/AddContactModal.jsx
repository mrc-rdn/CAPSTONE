...
export default function AddChapterModal(props) {
    ...
    const handleSubmit = async(e) =>{
      e.preventDefault();
      try {
        const addNewContacts = await axios.post(`${API_URL}/trainee/addContact`,{user2_id: id }, {withCredentials:true})
        console.log(addNewContacts.data.success)
        let success = addNewContacts.data.succcess
        let message = addNewContacts.data.message
        setIsContactAdded(success)
        setIsAdded(message)
      } catch (error) {
  ...
  return (
    <div className='w-full h-full bg-gray-500/40 fixed inset-0 flex items-center justify-center z-1000 '>
      <div className='w-10/12 h-5/12 bg-white p-3 rounded'>
